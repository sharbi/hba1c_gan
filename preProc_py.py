...
bp = pd.read_csv('./data/SPRINT/bp_with_hba1c.csv')
print(bp.head())
print(bp.shape)
...
for p in bp['MASKID'].unique():
    ...
    for j in range(len(visit_codes)):
        ...
        hba1c = bp.loc[(bp['MASKID'] == p) & (bp['VISITCODE'] == visit_codes[j])]['HBA1C']
...
# next set
print(bp['SBP'].max(), bp['DBP'].max(), bp['N_BPCLASSES'].max(), bp['HBA1C'].max())
...
hba1c_normalization_value = float(hba1c_max) / 2
...
for val in X_array:
    ...
    val[3] = (val[3] - hba1c_normalization_value) / hba1c_normalization_value
